...
def get_multi_class_index(answers):
    ...
    for ans in answers:
        ...
        if ans.startswith('A.'):
            ...
            match = re.search(r"([A-F])\.", ans)
            if match:
                cls_idx.append(ord(match.group(1)) - ord('A'))
            else:
                cls_idx.append(0)
    return cls_idx
...
def evaluate_model(test_loader, model, processor,device,option_vectors,vectorizer,options,option_labels,tokenizer):
    ...
    for inputs, batch_answers,fake_text_pos_list,captions,fake_image_box in tqdm(test_loader, desc="Evaluating"):
        ...
        true_multi = get_multi_class_index(batch_answers)
        pred_multi = best_option_indices.tolist()
        y_true_multi.extend(true_multi)
        y_pred_multi.extend(pred_multi)

        batch_true_binary = [0 if i == 0 else 1 for i in true_multi]
        batch_pred_binary = [0 if i == 0 else 1 for i in pred_multi]
        y_true_binary.extend(batch_true_binary)
        y_pred_binary.extend(batch_pred_binary)
        y_fake_score.extend((1.0 - similarities[:, 0]).tolist())
    ...
    binary_err = 1.0 - binary_acc if len(y_true_binary) > 0 else float("nan")
    if len(set(y_true_binary)) > 1:
        binary_auc = roc_auc_score(y_true_binary, y_fake_score)
    else:
        binary_auc = float("nan")

    multi_acc = accuracy_score(y_true_multi, y_pred_multi) if len(y_true_multi) > 0 else float("nan")
    macro_f1 = f1_score(y_true_multi, y_pred_multi, average='macro', zero_division=0) if len(y_true_multi) > 0 else float("nan")
    weighted_f1 = f1_score(y_true_multi, y_pred_multi, average='weighted', zero_division=0) if len(y_true_multi) > 0 else float("nan")

    return ACC_cls, cls_acc_all, cls_nums_all,IOU_score,MAP,Token_ACC,binary_acc,binary_err,binary_auc,multi_acc,macro_f1,weighted_f1
...
def main():
    ...
    for val_js in args.vals:
        ...
        ACC_cls, cls_acc_all, cls_nums_all, MAP, IOU_score, Token_ACC, binary_acc, binary_err, binary_auc, multi_acc, macro_f1, weighted_f1 = evaluate_model(test_loader,model,processor,device,option_vectors,vectorizer,options,option_labels,tokenizer)

        log_print('#######<--record-->###########')
        log_print(f"ACC_cls (Accuracy): {ACC_cls*100} (cls_acc_all: {cls_acc_all}, cls_nums_all: {cls_nums_all})")
        log_print(f"Binary_ACC: {binary_acc*100}")
        log_print(f"Binary_ERR: {binary_err*100}")
        log_print(f"Binary_AUC: {binary_auc*100 if not math.isnan(binary_auc) else 'nan'}")
        log_print(f"Multi_ACC: {multi_acc*100}")
        log_print(f"Multi_macro_F1: {macro_f1*100}")
        log_print(f"Multi_weighted_F1: {weighted_f1*100}")
