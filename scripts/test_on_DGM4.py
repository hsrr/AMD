...
def main():
    ...
    parser.add_argument("--vals", nargs="+", required=True, help="List of validation JSON/JSONL file paths")
    parser.add_argument("--output_file", type=str, default=None, help="File to save evaluation logs")
    parser.add_argument(
        "--image_root",
        type=str,
        default="",
    ...
    def log_print(*log_args, **kwargs):
        print(*log_args, **kwargs)
        with open(output_file, "a", encoding="utf-8") as flog:
            print(*log_args, **kwargs, file=flog)
